...
const SuprisePInnerPage = () => {
  ...
  useEffect(() => {
    const incrementOpenCount = async () => {
      const packageRef = doc(fireDB, "surpriseplannerpackages", pkg.id);
  ...
  }, [pkg.id]); // Only run when pkg.id changes
...
const handleFormSubmit = async (e) => {
  ...
  const handleBookingPage = () => {
    ...
    const bookingData = {
      ...
      price: packagePrice, // Send the total price instead of just pkg.price
      rating: pkg.ratings,
  ...
  return (
    <>
      ...
      <img src={mainImage} alt={pkg.name || 'Main View'} />
        ...
        <h1>{pkg.name}</h1>
        <p>This package has been opened times.</p>
