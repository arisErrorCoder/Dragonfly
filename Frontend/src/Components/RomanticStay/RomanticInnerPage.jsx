...
const RomanticInnerPage = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const packageRef = doc(fireDB, "surpriseplannerpackages", product.id);
...
const handleFormSubmit = async (e) => {
  ...
  const handleBookingPage = () => {
    ...
    const packagePrice = Number(product.price) || 0; // Ensure packagePrice is a number
    ...
    const bookingData = {
      ...
      addons: selectedAddons, // Send selected addons
    };
    ...
    <>
      ...
      <img src={mainImage} alt={product.name || 'Main View'} />
        ...
        <Carousel showThumbs={false}>
          ...
          <h4>{product.description || 'Designed to be a haven for the senses and an oasis of luxury.'}</h4>
...
export default RomanticInnerPage;
